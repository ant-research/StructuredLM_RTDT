...
from model.inside_outside_residual import InsideShareMLPLayer, OutsideShareMLPLayer
import math
...
class FastR2D2Plus(R2D2Base):
    def __init__(self, config, **kwargs):
        ...
        self.inside_layers = nn.ModuleList([InsideShareMLPLayer(config, self.inside_fn) for _ in range(self.iter_times)])
        self.outside_layers = nn.ModuleList([OutsideShareMLPLayer(config, self.outside_fn) for _ in range(self.iter_times)])
    ...
    def outside_fn(self, parent_ij, child_ikj, parent_scores, child_scores):
    ...
    def forward(self, 
        ...
        for iter_i in range(self.iter_times):
            ...
            splits_orders = self.inside(self.inside_layers[iter_i], inside_cache, 
            ...
            if pairwise:
                root_embedding, group_embedding = \
                    pairwise_contextual_inside_outside(root_embedding, self.inside_layers[iter_i], 
