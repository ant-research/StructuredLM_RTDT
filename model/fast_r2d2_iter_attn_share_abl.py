...
from utils.tree_utils import build_trees, pairwise_tgt_ids
...
class FastR2D2Plus(R2D2Base):
    ...
    def forward(self, 
        ...
        if tgt_ids is not None:
            cache_ids = cache_ids[:, :tgt_ids.shape[1]]

        # self attention
        if tgt_ids is not None:
            if pairwise:
                rebuilt_tgt_ids = pairwise_tgt_ids(cache_ids, tgt_ids)
                rebuilt_tgt_ids = rebuilt_tgt_ids.to(self.device, non_blocking=True)
            else:
                rebuilt_tgt_ids = tgt_ids
        ...
        if recover_tree:
            ...
            trees = build_trees(seq_lens_np, split_ids, None)
