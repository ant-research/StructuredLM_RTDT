...
from model.inside_outside_residual import InsideLayer, OutsideLayer
from functools import partial

from utils.tree_utils import build_trees, rebuild_tgt_ids
from .fast_r2d2_functions import group_contextual_inside_outside, rebuild_batch_indices_cpu
...
class FastR2D2Plus(R2D2Base):
    def __init__(self, config, **kwargs):
        ...
        self.inside_layers = nn.ModuleList([InsideLayer(config) for _ in range(self.iter_times)])
        self.outside_layers = nn.ModuleList([OutsideLayer(config) for _ in range(self.iter_times)])
        ...
        self.pair_classifier = nn.Linear(config.hidden_size, config.pairwise_label_num)
    ...
    def initialize_embeddings(self, input_ids, seq_lens, tgt_ids):
    ...
    def inside(self,
        ...
        for target_cache_ids, cache_ids, detach_cache_ids in inside_groups:
            ...
            scores_ijk, c_ijk = inside_layer(e_ij, parent_ij)
            ...
            if is_final_layer:
                best_splits.append(scores_ijk_sum.argmax(dim=1).to('cpu', non_blocking=True))

        return best_splits
        
    
    def outside(self, outside_layer, batch_size, root_ids, root_embedding, inside_cache, outside_groups):
        ...
        for target_cache_ids, cache_ids, _ in outside_groups:
            ...
            out_scores, out_ikj = outside_layer(parent_ij, child_ikj, parent_ij_score, child_scores)
    ...
    def forward(self, 
        ...
        for iter_i in range(self.iter_times):
            ...
            best_splits = self.inside(self.inside_layers[iter_i], inside_cache, 
            ...
            if group_ids is not None:
                root_embedding, group_embedding = \
                    group_contextual_inside_outside(root_embedding, self.inside_layers[iter_i], 
                                                       self.outside_layers[iter_i], group_embedding)

            outside_cache = self.outside(self.outside_layers[iter_i], batch_size, root_ids, root_embedding, inside_cache,
                                         zip(reversed(target_cache_ids), reversed(cache_ids), reversed(detach_cache_ids)))
        
        
        split_ids, cache_ids = tables.best_trees(best_splits)  # split_ids: -1 for terminal nodes, -100 for paddin
        # self attention
        if tgt_ids is not None:
        ...
        if pair_targets is not None:
            pair_logits = self.pair_classifier(outputs[:, 0, :])  # (N, C)
            pair_loss = F.cross_entropy(pair_logits, pair_targets)
        else:
            pair_loss = torch.zeros((1,), dtype=torch.float, device=self.device)

        results = {}
        # estimate cross entropy loss
        results['loss'] = [loss, pair_loss]
