...
from trainer.model_factory import create_fast_r2d2_model
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if step % log_step == 0 and step > 0:
                    with torch.no_grad():
                        self.model.eval()
                        self.logger.info(f"progress: {step}/{total_step} avg training loss: {loss}")#, training acc: {hits / labels.shape[0]}")
                        self.model.train()

            with torch.no_grad():
                self.model.eval()
                acc = evaluator.eval(eval_dataloader)
...
if __name__ == "__main__":
    ...
    cmd.add_argument("--task", type=str, default="intent")
    cmd.add_argument("--datasets", type=str, default="snips")
    cmd.add_argument("--data_dir", required=True, type=str)
    ...
    model = create_fast_r2d2_model(args.model_name, args.config_path, dataset.num_labels, args.pretrain_dir, args.output_dir)
