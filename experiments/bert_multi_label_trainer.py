...
from experiments.eval_multi_label_bert import MultiLabelEvaluater
...
if __name__ == "__main__":
    ...
    cmd.add_argument("--log_step", default=20, type=int)
    cmd.add_argument("--eval_step", default=40, type=int)
    cmd.add_argument("--datasets", type=str, required=True)
    cmd.add_argument("--task", type=str, choices=["NER", "intent"], default="NER")
    cmd.add_argument("--domain", type=str, default="navigate", help="stanfordLU datasets domain")
    cmd.add_argument("--model_name", required=True, type=str, help='args: bert_[dp/multilabel]*')
    ...
    if args.datasets == 'snips':
        dataset = SnipsReader(
            ...
            task=args.task,
            mode="train",
            enable_dp="dp" in args.model_name
        )
    ...
    model = create_bert_model(args.model_name, args.config_path, len(dataset.labels), None, None)
    # 模型内部实现了加载
    # if args.pretrain_dir is not None:
    #     load_model()
        # model_path = os.path.join(args.pretrain_dir, 'pytorch_model.bin')
