...
from utils.beam_parser import BeamParser
...
class R2D2ParserWrapper(TreeDecoderWrapper):
    def __init__(self, 
        ...
        if self.beam_size > 0:
            self.beam_parser = BeamParser(self._parser, self._model)
    ...
    def _parse(self, input_ids, attention_mask, atom_spans=None):
        if self.beam_size == 0:
            ...
            s_indices = self.beam_parser(input_ids, attention_mask, 
                                         atom_spans=atom_spans, 
                                         beam_size=self.beam_size)
            return [beam[0] for beam in s_indices]
    ...
    def __call__(self, tokens):
        ...
        with torch.no_grad():
            if not self._in_word:
                merge_trajectories = self._parse(**inputs)
